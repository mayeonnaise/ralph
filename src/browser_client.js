...
function refreshHaikus() {
  ...
  for (var block of blockchain.chain) {
    ...
    haikus[haiku.id] = haikus[haiku.id] === undefined ? {} : haikus[haiku.id];
    haikus[haiku.id][haiku.lineno] = haiku.line;
    haikuStatus[haiku.id] =
      haikuStatus[haiku.id] === undefined
        ? haiku.lineno
        : haikuStatus[haiku.id] < haiku.lineno
        ? haiku.lineno
        : haikuStatus[haiku.id];
  }
...
function createHaikuElement(haikuName, haiku) {
  ...
  haikuElement.onclick = function () {
    console.log(haiku);
    var haikuLinePairs = [];
    const selectedHaiku = haikus[haikuName];
    console.log(haikus);
    for (var haikuLineNo in selectedHaiku) {
      haikuLinePairs.push([haikuLineNo, selectedHaiku[haikuLineNo]]);
    }
    ...
    document.getElementById("blockchain").appendChild(submitdiv);
    for (var i = 0; i < haikuLinePairs.length; i++) {
      if (haikuLinePairs[i][0] >= 0 && haikuLinePairs[i][0] < 3) {
        console.log(document.getElementById("blockchain").children);
        var element = document.createElement("p");
        element.appendChild(document.createTextNode(haikuLinePairs[i][1]));
        document
          .getElementById("blockchain")
          .insertBefore(element, document.getElementById("submitdiv"));
      }
    }
    ...
    currHaiku = haikuName;
    console.log(haikuStatus);
    if (haikuStatus[currHaiku] >= 2) {
      document
        .getElementById("submitdiv")
...
function createContentPage(haikus) {
  haiku_ids = [];
  haiku_elements = [];
  document.getElementById("contentspage").innerHTML = "";
  console.log(haikus);
...
function parseMessage(event) {
  ...
  switch (data.type) {
    case "block": {
      ...
      if (block.payload.line !== "" && block.payload.id === currHaiku) {
        var element = document.createElement("p");
